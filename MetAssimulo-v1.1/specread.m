function [spec,ppm1,ppm2,bytordp] = specread(expname,expno,procno,disk,user)
...
if (dim==1)
    ...
    if ~(exist(pfile)==2) warning(sprintf('File %s does not exist',pfile)); ignore=1; end
    ...
    if ~(exist(pfile)==2) warning(sprintf('File %s does not exist',pfile)); ignore=1; end
    if ~(exist(p2file)==2) warning(sprintf('File %s does not exist',p2file)); ignore=1; end
    ...
    l = length(spec);
    if (si*si2 ~= l)
        warning(sprintf('Read dimensions %dX%d do not match spectrum size %d',si,si2,l));ignore=1; 
        return
    end
end   
